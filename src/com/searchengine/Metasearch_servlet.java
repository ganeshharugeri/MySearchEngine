...
public class Metasearch_servlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,
		...
		ArrayList<String> keys = new ArrayList<String>(Arrays.asList(names.split(" ")));
		...
		try {
			...
			for (Integer s : statcount)
				count.add(s);
			System.out.println("Groups after stat check in DB: >> "
					+ count.toString());
			...
			out.println("<div id=header><font color=blue><h4 align=centre> Search results for :"
					+ names + "</h4> </font></div>");
			out.println("<table border=1> <tr><th> Rank  </th><th> Search Engine </th> <th padding: 5px;>   URL    </th> <th>  Score   </th></tr>");
			StringBuilder metaquery = new StringBuilder();
			metaquery.append("select se_id,url,sum(dscore) as s from metaurldetails,to_tsquery('");
			for (int i = 0; i < keys.size(); i++) {
				if (i > 0) {
					metaquery.append("|");
				}
				metaquery.append(keys.get(i));
			}
			metaquery
					.append("') q where term @@q group by se_id,url order by s desc");
			...
			while (mets_rs.next() && z <= 20) {
				System.out.println("\n" + mets_rs.getString(2)
						+ " meta score: " + mets_rs.getFloat(3));
				out.print("<tr><td>" + z + "</td> <td> Group "
						+ mets_rs.getInt(1)
