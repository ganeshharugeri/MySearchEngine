...
public class JSONParser {
	...
	public void formURL(String names, Integer i) throws IOException {
		...
		case 1:
			...
			// url.append("http://192.168.18.11/Search/json");
			break;
		case 2:
		...
		try {
			...
			if (jsonObject != null) {
				...
				while (statItr.hasNext()) {
					...
					if(innerObj.get("term") != null && innerObj.get("df") != null){
					...
					if (!term.isEmpty()) {
						...
						System.out.println("List sending to store: " + list);
	...
	public HashSet<Integer> checkStat(String names) throws SQLException {
		...
		try {
			...
			se_select.append(" ') q where term @@ q group by se_id order by s desc limit 5");
