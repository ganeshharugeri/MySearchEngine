...
public class Search_Servlet_German extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (result != null) {
				out.println("<h3> Werbung:</h3>");
				System.out.println("result: " + result.size());

				for (int j = 0; j < result.size(); j = j + 4) {

					out.println("<p><b><font color='orange'><font size=4>Anzeige : </font></font>");
				...
				while (rs4.next() && z <= 20) {
					if (namesList.size() > 1) {
						out.print("<p><b><a href=" + rs4.getString(1) + "><font color='blue'><font size=4>"
								+ rs4.getString(2) + "</font></font></a></b>" + "<br><font color='green'>"
						...
						System.out.println("\n" + rs4.getString(2) + "\n" + rs4.getString(1) + "\n" + rs4.getString(3));
						out.print("<p><b><a href=" + rs4.getString(1) + "><a href=" + rs4.getString(1)
								+ "><font color='blue'><font size=4>" + rs4.getString(2) + "</a></font></font></a></b>"
								+ "<br><font color='green'>" + rs4.getString(1) + "</font>" + "<br>" + rs4.getString(3)
								+ "</br></p>");
			...
			if (!siteflag && !tildeflag && !andflag) {
				...
				StringBuilder orquery = new StringBuilder("select distinct(url), title");
				for (int i = 0; i < keys.size(); i++) {
					if (keys.size() > 1) {
						orquery.append(", ts_headline('german', pagecontent, to_tsquery('" + keys.get(i)
								+ "'), 'MaxWords=" + (int) (32 / keys.size()) + ", MinWords="
								+ (int) (32 / keys.size() - 4) + ", ShortWord=3')");
					} else
						orquery.append(",ts_headline('german', pagecontent, to_tsquery('" + keys.get(i)
				...
				orquery.append("')").append(
						" query where fs.language='german' and fs.docid= d.docid and fs.docid=i.docid and pagecontent @@ query order by pr desc, rank desc ");
				// commented to remove term in(?) type search to stem query
				// terms using to_tsquery'
				// orquery.append("')").append(" query where
				// fs.language='german' and fs.docid= d.docid and
				// fs.docid=i.docid and term in(");
				...
				while (rs1.next() && !andflag && z <= 20) {
					if (keys.size() > 1) {
						out.print("<p><b><a href=" + rs1.getString(1) + "><font color='blue'><font size=4>"
								+ rs1.getString(2) + "</font></font></a></b>" + "<br><font color='green'>"
			...
			if (andflag && !siteflag && !tildeflag) {
				...
				andquery.append(
						" '), 'MaxWords=16, MinWords=15, ShortWord=3'), fs.pr_score pr, ts_rank(vector, quotequery) AS rank from features fs JOIN documents d  USING (docid) JOIN snippets s USING (docid), to_tsquery ('");
				...
				andquery.append("') quotequery  where language='german' and pagecontent @@ quotequery ")
