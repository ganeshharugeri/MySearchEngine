		...
		try {
			...
			if (!siteflag && !tildeflag && !andflag) {
				...
				for (int i = 0; i < keys.size(); i++) {
					if (keys.size() > 1) {
						orquery.append(", ts_headline('english', pagecontent, to_tsquery('"
								+ keys.get(i)
								+ "'), 'MaxWords="
								+ (int) (32 / keys.size())
				...
				orquery.append("')")
						.append(" query where fs.language='english' and fs.docid= d.docid and fs.docid=i.docid and term in(");
				...
				while (rs1.next() && !andflag && z <= 20) {
					if (keys.size() > 1) {
						out.print("<p><b><font color='blue'><font size=4><a href="
								+ rs1.getString(1)
								+ ">"
								+ rs1.getString(2)
								+ "</a></font></font></b>"
			...
			if (andflag && !siteflag && !tildeflag) {
				...
				andquery.append(" '), 'MaxWords=16, MinWords=8, ShortWord=3'), ts_rank(vector, quotequery) AS rank from documents d JOIN snippets s USING (docid), to_tsquery ('");
				...
				andquery.append(
						"') quotequery  where pagecontent @@ quotequery ")
