...
//Class to Read Advanced Crawler options
public class Readcrawleroptions extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,
		...
		int shing_size = Integer.parseInt(request.getParameter("shing_size"));
		...
		try {
			...
			int recieve = send.startCrawling(host_url, max_depth,
					max_no_of_docs, domain, shing_size);
			if (recieve == -1) {
				...
				output.println("<h3>Time taken: " + ((totalTime / 1000) % 60)
						+ " seconds</h3>");
