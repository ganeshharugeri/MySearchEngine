...
public class Indexer {
	...
	String[] englishStopwordsArray = { "it", "every", "least", "less", "many",
			"now", "ever", "never", "say", "says", "said", "also", "get", "go",
			"goes", "just", "made", "make", "put", "see", "seen", "whether",
			"like", "well", "back", "even", "still", "way", "take", "since",
			"another", "however", "two", "three", "four", "five", "first",
			"second", "new", "old", "high", "long", "again", "further", "then",
			...
			"over", "under", "a", "an", "the", "oughtn't", "mightn't",
			"daren't", "needn't", "let's", "that's", "who's", "what's",
			"here's", "there's", "when's", "where's", "why's", "how's",
			"won't", "wouldn't", "shan't", "shouldn't", "can't", "cannot",
			"couldn't", "mustn't", "isn't", "aren't", "wasn't", "weren't",
			"hasn't", "haven't", "hadn't", "doesn't", "don't", "didn't", "i'm",
			...
			"they'll", "ought", "must", "may", "might", "could", "can",
			"should", "shall", "will", "would", "do", "does", "did", "doing",
			"have", "has", "had", "having", "am", "is", "are", "was", "were",
			"be", "been", "being", "what", "which", "who", "whom", "this",
			"that", "these", "those", "they", "them", "their", "theirs",
			"themselves", "i", "me", "my", "the", "myself", "we", "us",
			"usually", "our", "ours", "ourselves", "you", "your", "yours",
			"yourself", "yourselves", "he", "him", "his", "himself", "she",
			"her", "hers", "herself", "one", "its", "itself" };
	// German Stopwaords list
	String[] germanStopwordsArray = { "aber", "als", "am", "an", "auch", "auf",
			"aus", "bei", "bin", "bis", "bist", "da", "dadurch", "daher",
			"darum", "das", "dass", "dein", "deine", "dem", "den", "der",
			...
			"oder", "seid", "sein", "seine", "sich", "sie", "sind", "soll",
			...
			"werdet", "weshalb", "wie", "wieder", "wieso", "wir", "wird",
			"wirst", "wo", "woher", "wohin", "zu", "zum", "zur", "über" };

	// Function to Parse URL Content
	public boolean parse(String primURL, int docid) throws IOException,
		...
		// Send entire content to documents table
		Snippets.storeContents(docid, title, contents);

		// Create and store shingles in DB
		Shingles.createShingles(contents, docid);
		...
		// Entering Language Detection method
			...
			if (keys[i].length() != 1) // avoid storing single letter words
			{
				...
				if (!stopword) {
					...
					if (keywords.containsKey(stemmedKeyword)) {
						keywords.put(stemmedKeyword,
								keywords.get(stemmedKeyword) + 1);
					} else
	...
	public boolean LanguageDetection(String metadata,
			HashMap<String, Integer> englishStopwords) {
		double count = 1;
		String[] content = metadata.split(" ");
		// Entering the Language Detection Section
		for (int i = 0; i < content.length; i++) {
			// Deciding based on English Stopwords
			if (englishStopwords.containsKey(content[i])) {
				count++;
			}
		}
		// If there is english stopword occuring one in seven terms then
		// classify as English else German
